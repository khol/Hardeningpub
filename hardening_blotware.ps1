...
foreach ($App in $AppsToRemove) {
    if ($App -like "*`**") { # Check for wildcard
        $AppxPackages = Get-AppxPackage -AllUsers -Name $App
...
$startMenuShortcuts = Get-ChildItem -Path "C:\ProgramData\Microsoft\Windows\Start Menu\Programs" -Recurse | Where-Object {$_.Name -like "*$app*"}
$desktopShortcuts = Get-ChildItem -Path "$env:Public\Desktop" , "$env:userprofile\Desktop" | Where-Object {$_.Name -like "*$app*"}
